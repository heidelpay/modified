...
define('MODULE_HPIVSEC_CHECK_TEXT_DESCRIPTION', 'Prüfen Sie die Zahlungseingänge über Heidelpay gesicherte Rechnung');
define('MODULE_HPIVSEC_CHECK_TEXT_TITLE', 'Heidelpay gesicherte Rechnung Prüfung');
define('MODULE_HPIVSEC_CHECK_STATUS_DESC', 'Modulstatus');
define('MODULE_HPIVSEC_CHECK_STATUS_TITLE', 'Status');
define('HPIVSEC_IMAGE_EXPORT', 'Dr&uuml;cken Sie Ok um die offenen Rechnungszahlungen mit Heidelpay abzugleichen!');
define('HPIVSEC_IMAGE_EXPORT_TYPE', '<b>Abgleich der Rechnungszahlungen:</b>');
...
class hpiv_check
{
    ...
    public function __construct()
    {
        ...
        $this->enabled = ((MODULE_HPIVSEC_CHECK_STATUS == 'True') ? true : false);
        $this->hp = new heidelpay();
    }

    public function process()
    {
        $payment_class = 'hpivsec';
        $paystatus = MODULE_PAYMENT_HPIVSEC_PROCESSED_STATUS_ID;
        $finishstatus = MODULE_PAYMENT_HPIVSEC_FINISHED_STATUS_ID;
    ...
    $treeMonth = 34 * 24 * 60 * 60;
        ...
        if (!empty($res)) {
            ...
            if (count($xmlObject->Result->Transaction) > 0) {
                ...
                .' action="module_export.php?set=&action=edit&dowork=1&module=hpiv_check">'.$crlf;
          ...
          $i++;
                  ...
                  .$order['customers_id'].'&action=edit" target="_blank">'.$order['customers_name'].'</a></td>'.$crlf;
                  ...
                  .(string)$v->Payment->Clearing->Amount.' '.(string)$v->Payment->Clearing->Currency.'</td>'.$crlf;
                    $out.= '<td>'.$crlf;
                    $out.= '<input type="radio" name="book['.$i.'][status]" value="'
                  .$finishstatus.'" '.$checked.'> '.$finishName.'<br>'.$crlf;
                    $out.= '<input type="radio" name="book['.$i.'][status]" value="'
                  .$paystatus.'" '.$nochecked.'> '.$payName.$crlf;
                    $out.= '</td>'.$crlf;
                    $out.= '</tr>'.$crlf;
                }
                $out.= '</table>'.$crlf;
    ...
    {
        $out = array('text' => HPIVSEC_IMAGE_EXPORT_TYPE.'<br>'.
      HPIVSEC_IMAGE_EXPORT.'<br>'
      .xtc_button_link(
    ...
    public function check()
    {
        if (!isset($this->_check)) {
            $check_query = xtc_db_query("select configuration_value from " . TABLE_CONFIGURATION
                . " where configuration_key = 'MODULE_HPIVSEC_CHECK_STATUS'");
    ...
    public function install()
    {
        ...
        ." values ('MODULE_HPIVSEC_CHECK_STATUS', 'True',  '6', '1',"
    ...
    {
        return array();
        return array('MODULE_HPIVSEC_CHECK_STATUS');
    }
}
