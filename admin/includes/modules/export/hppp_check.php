...
class hppp_check
{
    ...
    $crlf = "\r\n";
      ...
      if (count($xmlObject->Result->Transaction) > 0) {
        ...
        $out.= '</form><form method="post" action="module_export.php?set=&action=edit&dowork=1&module=hppp_check">'.$crlf;
...
#    echo '<pre>'.print_r($realAmounts, 1).'</pre>';
    ...
    {
        $out = array('text' => HPPP_IMAGE_EXPORT_TYPE.'<br>'.
    ...
    public function check()
    {
        if (!isset($this->_check)) {
            $check_query = xtc_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_HPPP_CHECK_STATUS'");
    ...
    {
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value,  configuration_group_id, sort_order, set_function, date_added) values ('MODULE_HPPP_CHECK_STATUS', 'True',  '6', '1', 'xtc_cfg_select_option(array(\'True\', \'False\'), ', now())");
    }
