...
class heidelpay {
	var $response = '';
	var $error = '';
	var $hpdebug = false; // um Debugging zu aktivieren auf true setzten
	...
	function handleDebit($order, $payCode, $insertId = false) {
		...
		if ($user_id <= 0)
			$user_id = $_SESSION ['customer_id']; // Fallback User Id
		$orderId = 'User ' . $user_id . '-' . date ( 'YmdHis' );
		if (! empty ( $insertId ))
		...
		if (in_array ( strtoupper ( $payCode ), array (
			...
			if ($payMethod == 'DB')
				$payMethod = 'PA'; // Rechnung und Vorkasse immer PA
			if (in_array ( strtoupper ( $payCode ), array (
					'PP',
					'IV' 
			) ))
		...
		if ($res ['all'] ['PROCESSING.STATUS.CODE'] == '80' && $res ['all'] ['PROCESSING.RETURN.CODE'] == '000.200.000' && $res ['all'] ['PROCESSING.REASON.CODE'] == '00') {
			...
			if (@constant ( 'MODULE_PAYMENT_HP' . strtoupper ( $this->actualPaymethod ) . '_DIRECT_MODE' ) == 'GAMBIOLIGHTBOX') {
				...
				exit ();
			} else if (@constant ( 'MODULE_PAYMENT_HP' . strtoupper ( $this->actualPaymethod ) . '_DIRECT_MODE' ) == 'LIGHTBOX') {
		...
		if (in_array ( $payCode, array (
		...
		} else if (in_array ( $payCode, array (
				'pp',
				'iv' 
		) ) && $ACT_MOD_MODE == 'AFTER') {
			...
			// BarPay
		} else if (! empty ( $res ['all'] ['CRITERION_BARPAY_PAYCODE_URL'] )) {
	...
	function prepareData($orderId, $amount, $currency, $payCode, $userData, $lang, $mode = 'DB', $capture = false, $uniqueId = NULL) {
		...
		} else if ($this->actualPaymethod == 'IDL' && $ACT_MOD_MODE == 'NOWPF') {
	...
	function doRequest($data, $xml = NULL) {
		$url = $this->demo_url_new;
		if (! empty ( $xml ))
			$url = 'https://test-heidelpay.hpcgw.net/TransactionCore/xml'; // XML
		if (constant ( 'MODULE_PAYMENT_HP' . $this->actualPaymethod . '_TRANSACTION_MODE' ) == 'LIVE') {
			$url = $this->live_url_new;
			if (! empty ( $xml ))
				$url = 'https://heidelpay.hpcgw.net/TransactionCore/xml'; // XML
		}
		$this->url = $url;
	 ...
	 */
	function getQueryXML($dateFrom, $dateUntil, $types = array('RC'), $identification = array(), $methods = array()) {
		$parameters = array ();
		$parameters ['SECURITY.SENDER'] = constant ( 'MODULE_PAYMENT_HP' . $this->actualPaymethod . '_SECURITY_SENDER' );
		$parameters ['USER.LOGIN'] = constant ( 'MODULE_PAYMENT_HP' . $this->actualPaymethod . '_USER_LOGIN' );
		$parameters ['USER.PWD'] = constant ( 'MODULE_PAYMENT_HP' . $this->actualPaymethod . '_USER_PWD' );
	...
	function getHistoryComment($order_id, $search) {
	...
	function saveIds($uniqueId, $order_id, $paymeth, $shortId) {
  ...
  KEY `orderId` (`orderId`))";
	...
	function saveOrderComment($order_id, $comment) {
		return xtc_db_query ( "UPDATE `" . TABLE_ORDERS . "` SET `comments` = '" . addslashes ( $comment ) . "' WHERE `orders_id` = '" . addslashes ( $order_id ) . "'" );
	}

	
	function install_db($tablename, $fieldname, $add_fieldspec, $setindex = false) {
	...
	function getOpenOrdersDate($payment_class, $paystatus) {
	...
	function getOpenOrders($payment_class, $paystatus) {
	...
	function getOpenOrderByUniqueId($uniqueId, $payment_class) {
...
WHERE heidelpay_transaction_data.uniqueID = "' . $uniqueId . '"
	...
	function checkOrderStatusHistory($orderId, $shortId) {
	...
	function saveBankData($customerId, $payType, $values) {
	...
	function loadBankData($customerId, $payType) {
	...
	function rememberOrderData($order) {
		...
		if (empty ( $userData ['state'] ))
			$userData ['state'] = $userData ['country']; // Wenn Bundesstaat leer dann nimm Land
	...
	function trackStep($point, $var, $val) {
	...
	function deleteCoupon($order_id) {
